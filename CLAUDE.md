...
- `crates/chess-server`: `hub` owns rooms, seats, clocks, eviction, and persistence and never touches a socket; `store` is the SQLite database (accounts, sessions, SSH keys, finished games; schema changes are appended to `MIGRATIONS`, never edited); `accounts` adds validation, Argon2id hashing, and session tokens on top and is async so hashing never runs under a lock; `server` is the axum transport, which answers account commands itself and passes the rest to the hub; `main.rs` only reads the environment.
