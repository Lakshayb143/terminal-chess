...
The modules are declared in `main.rs`. `board` is the base, `movegen` depends on it, `san` and `eval` depend on both, `search` owns engine search, and `ui` owns terminal rendering and layout.
...
**`board.rs`: position representation**
