...
- `crates/chess-core`: `board` is the base, `movegen` depends on it, `san` and `eval` depend on both, `search` owns engine search, and `game` holds UI-independent game state and outcomes. `tests/rules.rs` has perft, hashing, and notation round-trip tests; run them after touching move generation. `tests/search.rs` checks the engine finds what it should and strays only as far as `Limits::randomness` allows.
...
- `crates/chess-server`: `hub` owns rooms, seats, clocks, pairing strangers and the lobby count, rematches (a new room, colours swapped, once both players ask), eviction, and persistence, and never touches a socket; `store` is the SQLite database (accounts, sessions, SSH keys, finished games; schema changes are appended to `MIGRATIONS`, never edited); `accounts` adds validation, Argon2id hashing, and session tokens on top and is async so hashing never runs under a lock; `server` is the axum transport, which answers account commands itself and passes the rest to the hub; `ssh` is the SSH gateway, which runs the terminal client in a pseudo-terminal per visitor with `CHESS_HOSTED=1` (file commands off) and hands it a session or an SSH-key link ticket through the environment; `main.rs` only reads the environment.
- Root package: `ui` owns terminal rendering and layout, `input` raw keys and mouse (arrows, Home/End, Page Up/Down and the wheel are separate actions), `client` the WebSocket transport, and `storage` local files. `src/app` is the application: `local` runs the loop from the home page to a game and back (every game ends on the home page; only `q` there or Ctrl+C ends the program), `home` the home page, `settings` engine levels, clock presets and the Settings page, `online` online games, `screen` the frame, move review and the board cursor, and `account` the account pages. `tests/online.rs` runs a real server in-process against two clients.
...
- `Limits::randomness` weakens it: after the search, every root move is scored exactly at the completed depth and gets a random bonus of up to that many centipawns before the best is played. The app's `Level`s (`src/app/settings.rs`) are depth plus randomness; re-measure them against each other if either changes.
